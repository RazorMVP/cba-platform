// web-react/src/app/features/products/recurring-deposits/RecurringDepositDetailPage.tsx
import { useState } from 'react'
import { useParams, useNavigate } from 'react-router-dom'
import { PageHeader } from '@/shared/components/PageHeader'
import { StatusBadge } from '@/shared/components/StatusBadge'
import { Modal } from '@/shared/components/Modal'
import { useRecurringDepositProduct, useCreateRecurringDepositProduct, useUpdateRecurringDepositProduct, useDeleteRecurringDepositProduct } from '../api/useRecurringDeposits'
import type { CreateRecurringDepositProductRequest, RepaymentFrequencyType } from '../api/types'
...
export default function RecurringDepositDetailPage() {
  ...
  const { data, isLoading } = useRecurringDepositProduct(id!)
  const product = data?.data

  const create = useCreateRecurringDepositProduct()
  const update = useUpdateRecurringDepositProduct(id!)
  const del = useDeleteRecurringDepositProduct()

  const [editMode, setEditMode] = useState(isNew)
  const [form, setForm] = useState<CreateRecurringDepositProductRequest>(blank())
  const [tab, setTab] = useState<'core' | 'rates' | 'frequency' | 'term' | 'penalty'>('core')
  const [showDelete, setShowDelete] = useState(false)
  const [error, setError] = useState('')

  function blank(): CreateRecurringDepositProductRequest {
    return { name: '', shortName: '', description: '', currencyCode: 'USD', nominalAnnualInterestRate: 0, minDepositAmount: 0, recurringDepositFrequency: 1, recurringDepositFrequencyType: 'MONTHS' }
  }
  ...
  function enterEditMode() {
    ...
    setForm({
      ...
      minDepositAmount: product.minDepositAmount, recurringDepositFrequency: product.recurringDepositFrequency,
      recurringDepositFrequencyType: product.recurringDepositFrequencyType,
  ...
  async function handleDelete() {
    try { await del.mutateAsync(id!); navigate('/products/recurring-deposits') }
  ...
  if (isNew || editMode) {
    return (
      <div>
        <PageHeader title={isNew ? 'New Recurring Deposit Product' : `Edit: ${product?.name}`}
        ...
        <div className="rounded-xl p-6 space-y-4" style={{ background: 'var(--bg-card)', border: '1px solid var(--color-border)' }}>
          ...
          <Field label="Min Deposit Amount" type="number" min="0" step="0.01" value={String(f.minDepositAmount)} onChange={e => setForm(p => ({ ...p, minDepositAmount: parseFloat(e.target.value) || 0 }))} required />
          <p className="text-xs font-semibold uppercase tracking-wider pt-2" style={{ color: 'var(--color-muted)' }}>Deposit Frequency</p>
          <div className="grid grid-cols-2 gap-4">
            <Field label="Every" type="number" min="1" value={String(f.recurringDepositFrequency)} onChange={e => setForm(p => ({ ...p, recurringDepositFrequency: parseInt(e.target.value) || 1 }))} required />
            <Select label="Period" value={f.recurringDepositFrequencyType} onChange={v => setForm(p => ({ ...p, recurringDepositFrequencyType: v as RepaymentFrequencyType }))}
              options={[{ label: 'Months', value: 'MONTHS' }, { label: 'Weeks', value: 'WEEKS' }, { label: 'Days', value: 'DAYS' }]} />
          </div>
          <p className="text-xs font-semibold uppercase tracking-wider pt-2" style={{ color: 'var(--color-muted)' }}>Term (optional)</p>
          <div className="grid grid-cols-2 gap-4">
            <Field label="Min Term (months)" type="number" min="1" value={String(f.minDepositTerm ?? '')} onChange={e => setForm(p => ({ ...p, minDepositTerm: parseInt(e.target.value) || undefined }))} />
            <Field label="Max Term (months)" type="number" min="1" value={String(f.maxDepositTerm ?? '')} onChange={e => setForm(p => ({ ...p, maxDepositTerm: parseInt(e.target.value) || undefined }))} />
          </div>
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <span className="text-xs font-mono px-2 py-0.5 rounded" style={{ background: 'var(--bg-subtle)', color: 'var(--color-muted)' }}>Recurring Deposit</span>
      ...
      <div className="flex gap-1 mb-6 border-b" style={{ borderColor: 'var(--color-border)' }}>
        {(['core', 'rates', 'frequency', 'term', 'penalty'] as const).map(t => (
      ...
      {tab === 'frequency' && (
        <Grid>
          <Row label="Deposit Frequency" value={`Every ${product!.recurringDepositFrequency} ${FREQ_LABEL[product!.recurringDepositFrequencyType] ?? product!.recurringDepositFrequencyType}`} />
        </Grid>
      )}
      {tab === 'term' && (
        <Grid>
          <Row label="Min Term" value={product!.minDepositTerm ? `${product!.minDepositTerm} months` : '—'} />
      ...
      <Modal open={showDelete} onClose={() => setShowDelete(false)} title="Delete Recurring Deposit Product" size="sm"
        footer={
