...
- **card-service unit suite (Session 120 cont. 2): 65 tests** across the money-critical core — `CardAuthorizationServiceTest` (auth decision tree via PREPAID path), `FraudEngineTest` (scoring + hard-blocks + per-currency thresholds), `CardServiceTest` (lifecycle + PIN-retry + CoB expiry + PAN hash), `SettlementServiceTest`, `BinServiceTest`, `ApiKeyServiceTest`, `CavvGeneratorTest`, `TokenServiceTest`, `CardLimitServiceTest`, `DisputeServiceTest`. Run: `cd card-service && ./mvnw -o test`.
