...
| **Last git commit** | Session 120 cont. 8 (`13b4df2`) | Session 120 (cont. 8) — backend test-coverage hardening: 2 latent defects fixed (`Payment` NOT-NULL currency audit columns via `@PrePersist`; `FieldEncryptor` `DEMO_ENC:` decrypt passthrough), singleton Testcontainer, `OpenApiSnapshotTest` permit + canonicalization; `*IT` wired into `-Pfull-integration` (629 green) |
