...
- **Still deferred to integration tests:** DEBIT/CREDIT balance paths (monolith `RestTemplate` calls), the `@SpringBootTest` snapshot/Testcontainers tests (`-Pfull-integration`), and the larger services not yet unit-covered (`InterchangeQualificationEngine`, `ThreeDsService`, `BureauService`, `SpendingAnalyticsService`, `WebhookDeliveryService`, `TerminalSimulatorService`, `SettlementFileExportService`).
