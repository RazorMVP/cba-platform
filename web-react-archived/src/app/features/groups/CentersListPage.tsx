// web-react/src/app/features/groups/CentersListPage.tsx
import { useState } from 'react'
import { useNavigate } from 'react-router-dom'
import { PageHeader } from '@/shared/components/PageHeader'
import { StatusBadge } from '@/shared/components/StatusBadge'
import { useCenters, useCreateCenter } from './api/useGroups'
import type { Center, CenterStatus, CreateCenterRequest } from './api/types'

function statusVariant(s: CenterStatus): 'warning' | 'success' | 'neutral' {
...
function CreateCenterModal({ onClose }: { onClose: () => void }) {
  ...
  const [form, setForm] = useState<CreateCenterRequest>({ name: '', officeId: '', externalId: '', staffId: '' })
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.5)' }}>
      <div className="w-full max-w-md rounded-2xl p-6" style={{ background: 'var(--bg-card)' }}>
        <h2 className="text-base font-semibold mb-4" style={{ color: 'var(--color-text)' }}>New Center</h2>
        <div className="space-y-3">
          {([
            { label: 'Center Name', key: 'name'       as const, required: true  },
        ...
        {create.isError && (
          <p className="text-xs mt-3" style={{ color: 'var(--color-error)' }}>Failed to create center.</p>
        )}
...
export default function CentersListPage() {
  ...
  const centers: Center[] = (data as { data?: Center[] } | undefined)?.data ?? []
  ...
  const filtered = centers.filter(c => {
    ...
    if (search) {
      ...
      return c.name.toLowerCase().includes(q) || c.officeName.toLowerCase().includes(q)
    }
    return true
  })
  ...
  return (
    <div>
      <PageHeader
        title="Centers"
        subtitle="Collection meeting centers"
        actions={
      ...
      <div className="flex gap-3 mb-4">
        ...
        <select
          value={statusFilter}
          onChange={e => setStatusFilter(e.target.value as CenterStatus | '')}
          className="px-3 py-2 rounded-lg text-sm outline-none"
          style={{ background: 'var(--bg-subtle)', border: '1px solid var(--color-border)', color: 'var(--color-text)' }}
        >
        ...
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            ...
            {filtered.map(c => (
              ...
              >
                <td className="px-4 py-3 font-medium text-sm" style={{ color: 'var(--color-text)' }}>{c.name}</td>
                <td className="px-4 py-3 text-xs" style={{ color: 'var(--color-muted)' }}>{c.officeName}</td>
                <td className="px-4 py-3 text-xs" style={{ color: 'var(--color-muted)' }}>{c.staffName ?? '—'}</td>
                <td className="px-4 py-3 text-xs tabular-nums" style={{ color: 'var(--color-muted)' }}>
                  {c.activationDate ? new Date(c.activationDate).toLocaleDateString() : '—'}
                </td>
      ...
      {creating && <CreateCenterModal onClose={() => setCreating(false)} />}
    </div>
  )
}
