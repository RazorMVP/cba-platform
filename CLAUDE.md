...
- **fep-service unit suite: 64 tests** (Sessions 120 + cont. 3–4) — EMV (parser/ARQC/ARPC), scheme routing, MTI dispatch, auth DTOs, all four message handlers (`NetworkHandlerTest`, `AuthorizationHandlerTest`, `FinancialHandlerTest`, `ReversalHandlerTest`), and `FepSocketRoundTripTest` (real TCP round trip through the Netty pipeline). **fep-service Mockito was also unblocked on Java 25** (Session 120 cont. 3): SB 3.2.5's managed Mockito/Byte Buddy predate Java 25, so `fep-service/pom.xml` overrides `mockito.version=5.17.0` + `byte-buddy.version=1.17.6` and adds the surefire `-javaagent`/experimental `argLine` (same fix as card-service).
...
- **jPOS external-DTD startup risk (open):** the packager XMLs (`iso8583-*.xml`) declare `<!DOCTYPE … "http://jpos.org/dtd/packager.dtd">`, and jPOS 2.1.9's `GenericPackager(InputStream)` fetches it over the network with validation on (no local-resolver/validation-off hook). A network-isolated FEP could fail to boot. `FepSocketRoundTripTest` sidesteps this by using the code-based `ISO87APackager`. A proper fix (bundled DTD + XML catalog, code-based packagers, or jPOS upgrade) is a future hardening item.
...
- **Testcontainers can't run on this dev host:** Docker Desktop **29.5.2** (API 1.54) is installed and the `docker` CLI works, but Testcontainers 1.20.4's docker-java gets `HTTP 400` on the `/info` ping (incompatible with this very new Docker Desktop; `DOCKER_HOST`/`DOCKER_API_VERSION`/Ryuk-off/TC-1.21.3 all fail the same way). `-Pfull-integration` tests run in CI (standard Docker). To verify locally, start PostgreSQL via the Docker CLI, load the migrations, and run SQL directly (as was done for the Gap-7 query).
- **Still deferred to integration tests:** DEBIT/CREDIT balance *approve* paths (private `BalanceResponse` record), and remaining `@SpringBootTest`/Testcontainers coverage.
