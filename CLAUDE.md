...
- **`publishEvent` must be called cross-bean** to keep its `@Async` (self-invocation blocks the request thread). OB call sites use the static `PartnerWebhookDeliveryService.parseOrg(tppClientId)` + a cross-bean `publishEvent`. Events now fire from `PartnerService` (APPLICATION.*/API_KEY.*), `ConsentService` (CONSENT.CREATED/AUTHORISED/REVOKED/**EXPIRED** + FUNDS.CONFIRMED), `PispController` (PAYMENT.INITIATED/COMPLETED/FAILED). **CONSENT.EXPIRED wired Session 121 cont. 5** — `ConsentExpiryJob` (`@Scheduled` hourly `${app.openbanking.consent-expiry-cron:0 7 * * * *}`) → `ConsentService.expireDueConsents(now)` transitions AWAITING/AUTHORISED consents past `expiryDate` to EXPIRED, audits, fires the event. Still deferred (each needs new structure, not a quick wire): PAYMENT.REVERSED (no `Payment`→partner-org link), AUTHORIZATION.REVERSED (no card-service reversal handler), ACCOUNT.* (no clean trigger), RATE_LIMIT.* (filter runs before partner auth → no orgId).
