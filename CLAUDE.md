...
|-----------|---------|-------|
| **Spring Boot** | **3.5.16** | Parent BOM; governs Flyway, Hibernate, security versions. Bumped from 3.5.0 (Session 125 cont. 6). ⚠️ Six libraries are **pinned ahead of this BOM** for CVEs it still ships — Tomcat 10.1.60, Netty 4.1.138.Final, PostgreSQL 42.7.13, httpcore5 5.4.3, httpclient5 5.6.4, log4j2 2.26.1 (cont. 8). **Drop each pin when the BOM catches up**; see the "Security pins" comment in `backend/pom.xml` |
...
- **`publishEvent` must be called cross-bean** to keep its `@Async` (self-invocation blocks the request thread). OB call sites use the static `PartnerWebhookDeliveryService.parseOrg(tppClientId)` + a cross-bean `publishEvent`. Events now fire from `PartnerService` (APPLICATION.*/API_KEY.*), `ConsentService` (CONSENT.CREATED/AUTHORISED/REVOKED/**EXPIRED** + FUNDS.CONFIRMED), `PispController` (PAYMENT.INITIATED/COMPLETED/FAILED), and `PaymentReversalPartnerNotifier` (**PAYMENT.REVERSED**). **CONSENT.EXPIRED wired Session 121 cont. 5** — `ConsentExpiryJob` (`@Scheduled` hourly `${app.openbanking.consent-expiry-cron:0 7 * * * *}`) → `ConsentService.expireDueConsents(now)` transitions AWAITING/AUTHORISED consents past `expiryDate` to EXPIRED, audits, fires the event. **PAYMENT.REVERSED wired Session 121 cont. 6** — no migration needed: PISP payments carry actor `"open-banking:{consentId}"` (`PaymentService.transfer`), so the reversed payment's `createdBy` IS the consent pointer. `PaymentService.reversePayment` publishes a plain `PaymentReversedEvent`; `PaymentReversalPartnerNotifier` (`@Async @TransactionalEventListener(AFTER_COMMIT)`, in `openbanking` to avoid a `payment→openbanking` cycle) resolves the org from `createdBy` and fires the webhook (no-op for non-PISP/teller reversals). **RATE_LIMIT.WARNING/EXCEEDED wired Session 121 cont. 9** — the "filter runs before partner auth → no orgId" blocker is solved by resolving the org *directly from the request* instead of the SecurityContext: `RateLimitEventNotifier` (called by `RateLimitFilter` after the check) reads the partner JWT's `orgId` claim (unverified base64, same as the filter's identity extraction) or does an API-key→org lookup (`PartnerApiKeys.hash` → `findByKeyHashAndActiveTrue`). It fires only when a threshold is crossed — EXCEEDED on 429, WARNING in the last 10% of the window — and **only once per window** (`RateLimitService.firstEventInWindow`, Redis SETNX with 60s TTL, dedup keyed on the filter's existing `identity`), so the hot path pays nothing and a partner hammering over-limit gets one event per window, not a storm. Dedup runs BEFORE the org lookup, so the DB hit is at most once/window. Fail-safe: Redis down → SETNX returns false → event suppressed (no dedup guarantee = stay quiet). Still deferred (logged with full pick-up context in **`docs/deferred-backlog.md`**, Session 121 cont. 10): **ACCOUNT.ACCESS_GRANTED/BALANCE_UPDATED** (BALANCE_UPDATED is effectively a fan-out subscription — no account→consenting-orgs index, balance changes in many places, high volume), plus two non-webhook items — **async external-payment settlement lifecycle** (L, money path) and **full-PAN decrypt in settlement export** (M, PCI sign-off required). (AUTHORIZATION.REVERSED — a card-service webhook event, not a backend partner event — was wired Session 121 cont. 7; see the card-service section.)
...
- **card-service events wired:** CARD.EXPIRED (CoB), CARD.LIMIT_CHANGED (CardApiController), FRAUD.RULE_TRIGGERED/CARD_STEP_UP/CARD_DECLINED_HIGH_RISK (CardAuthorizationService), DISPUTE.RAISED/RESOLVED (DisputeService), **AUTHORIZATION.REVERSED** (`CardAuthorizationService.reverse` — Session 121 cont. 7). **AUTHORIZATION.REVERSED wired Session 121 cont. 7** — net-new card-service reversal handler `POST /api/v1/internal/reverse` (the FEP `ReversalHandler` MTI 0400/0420 already called `cardServiceClient.reverse(...)`, but card-service had no endpoint). It's a **record-and-notify, idempotent** op: locate the original auth by (card, STAN), guard on an existing 0400, record a 0400 entry, fire `AUTHORIZATION.REVERSED`. Financial reversal is a deliberate no-op — card-service auth/advice are check-and-record only (no funds move on the auth path), so there's nothing to return; this is the seam a real fund-return hooks into. **⚠️ Latent bug found + FIXED (cont. 7 reverse, cont. 8 the rest):** the FEP `CardServiceClient` posted to `/api/v1/fep/*` but card-service serves `/api/v1/internal/*` — so NO FEP→card-service HTTP call reached its target (both sides mock each other in tests, so it was never caught). **cont. 8 aligned all remaining calls**: FEP `authorize`/`advice`/`detokenize` URLs → `/api/v1/internal/{authorize,advise,detokenize}`; card-service `detokenize` changed GET+`@RequestParam` → **POST+`@RequestBody`** (keeps the DPAN out of access/proxy logs); `CardAuthRequest.emvTags` renamed → **`schemeData`** to actually mirror the FEP `AuthorizationRequest` (the field was silently dropped — both unused, so no behaviour change, but now a true mirror). New `FepAuthorizeContractTest` (2) serializes the FEP's exact authorize body → deserializes into `CardAuthRequest` and asserts every used field binds (+ response fields the FEP reads) — the contract test that was missing, which would have caught both the path and field mismatches. BIN calls (`/api/v1/bins/*`) were always correct.
...
- **Env note (Session 119):** local JDK is Java 25 — JaCoCo 0.8.12 can't instrument it (run tests with `-Djacoco.skip=true`). **SpotBugs 4.8.6.6 is the same story _(Session 125 cont. 9)_:** `mvn spotbugs:check` dies locally with `IllegalArgumentException: Unsupported class file major version 69` (= Java 25) while scanning even `java/lang/Object`, so it reports `Java returned: 1` rather than findings. **SpotBugs findings can only be verified in CI (Java 21)** — use `-Dspotbugs.skip=true` locally, and read the `SpotBugs Static Analysis` job on the PR for the real result.
- **MinIO images come from quay.io, NOT Docker Hub _(Session 125 cont. 9)_.** MinIO stopped publishing free Docker Hub images, so `minio/minio` now fails with `pull access denied … repository does not exist` — which broke `S3StorageProviderIntegrationTest` and therefore the **entire backend `test` job** on every run (703/704 passing). `quay.io/minio/minio` still serves the community releases; the test pins `RELEASE.2025-09-07T16-13-09Z` rather than a floating `latest`. ~~card-service's Mockito can't mock concrete classes (`Could not modify all classes`)~~ **RESOLVED Session 120** — card-service Mockito now mocks concrete classes on Java 25 via surefire `-javaagent:mockito-core` + `-Dnet.bytebuddy.experimental=true` (see "card-service Test Toolchain" below). card-service unit tests ARE runnable on this host.
...
**⚠️ The audit gate structurally deadlocks Dependabot — expect this to recur.** Dependabot bumps only within its **configured group** (`angular`, `angular-material` per `dependabot.yml`), so each PR leaves out-of-group advisories in place. `npm audit --audit-level=high` is all-or-nothing, so a partial fix still exits 1 → `security` fails → `deploy` is skipped → the PR never merges. Evidence: runs `32685408788` and `32685348141` (2026-08-24) both passed `Lint & Test` and failed `Security Audit`; the angular-group PR reached `6 vulnerabilities (1 low, 5 high)` and still exited 1. Advisories then compounded 6 → 19 over ~3 weeks with no merge path. **So: when a Dependabot PR for `web/` fails CI, don't retry it — do a full clean re-resolution instead** (procedure above), which fixes every advisory at once. The deadlock re-forms the next time a single out-of-group advisory lands, so consider re-scoping the gate (`--omit=dev`, or advisory `continue-on-error` with Dependabot/Snyk/Trivy as the tracking layers). *Changing a security gate is a maintainer decision — not a side effect of a dependency bump.*

**Browser-probe caveats when verifying a production build** (learned S124 while re-verifying the zone.js bump):
- Terser mangles class names, so `Promise.name === 'ZoneAwarePromise'` and `Promise.toString().includes('ZoneAwarePromise')` are **false negatives** in prod. Assert `Promise.name !== 'Promise'` instead (native reports `"Promise"`; a patched global reported `"k"`).
- `Zone.current.name` sampled from an injected script **always reads `<root>`**, even in a healthy zone-based app — tasks inherit the zone they were *scheduled* in, and an injected script schedules from root. Don't use zone-name sampling as a pass/fail signal.
- The reliable pair: **`!!XMLHttpRequest.prototype.send[Zone.__symbol__('OriginalDelegate')]`** (the mechanism by which `HttpClient` callbacks re-enter the Angular zone) **plus a functional render check** — does a data screen reach its post-async resolved state with `skeletons: 0, spinners: 0` and zero user interaction?
...
| File | Contents |
|------|----------|
| `docs/deferred-backlog.md` | **Known future work** — the 3 intentionally-deferred features with pick-up context (async external-payment settlement, full-PAN settlement decrypt, ACCOUNT.* webhooks) _(Session 121 cont. 10)_ |
| `docs/integration-runbook.md` | Credential-flip runbook — every external integration, its env vars, and go-live steps _(Session 121 cont. 1)_ |
...
>
> **⚠️ Trivy FS + Maven Central 429 _(Session 125 cont. 3)_.** Trivy fetches missing transitive POMs from `repo.maven.apache.org`; Maven Central rate-limits shared GitHub runner IPs (`429`, `Retry-After: 1800`) and Trivy fails fast by design. **Re-running is not a fix.** `trivy-fs` now pre-populates `~/.m2` (`setup-java` `cache: maven` + per-module `mvn dependency:resolve`) and scans with `TRIVY_OFFLINE_SCAN: 'true'`, which skips remote POM fetches only (vuln-DB download unaffected). Keep the pre-resolve step: offline mode *silently* drops any POM not in the cache. If a new Java module is added, add it to both the cache key and the resolve loop.
>
> **⚠️ OWASP Dependency Check — NVD cache must use restore/save split _(Session 125 cont. 5)_.** Plain `actions/cache` saves only when the **job succeeds**, and `owasp-check` fails on any CVSS ≥ 7 finding, so the NVD cache was **never written**. Every card-service run then paged all ~381k NVD records through the API (~40 min; sometimes never finished). Both `backend-ci.yml` and `card-service-ci.yml` now use `actions/cache/restore` + `actions/cache/save` with `if: always()`, a per-day key `owasp-nvd-Linux-YYYY-MM-DD` shared by both jobs, the project's public NVD mirror (`-DnvdDatafeedUrl=https://dependency-check.github.io/DependencyCheck_Builder/nvd_cache/`), the plugin pinned at `13.0.0`, and a 45-minute timeout. Two more traps: `hoursToWaitBeforeChecking` **is not a real property** (it's silently ignored; use `nvdValidForHours`), and the OSS Index analyzer now needs credentials, so it's disabled explicitly. **The gate stays BLOCKING at CVSS 7 — do not make it advisory _(Session 125 cont. 8)_.** Making `owasp-check` `continue-on-error` was attempted and reverted: an automated security review caught that the stated compensating controls do not exist. **`snyk`'s scan steps are `continue-on-error: true` and the Trivy scans set no `exit-code`, so both only report**, and Dependabot raises alerts rather than gating. That makes `owasp-check` the **only** CI gate that fails a build on a CVE in an *existing* Java dependency (`dependency-review` sees only deps *changed in a PR*; `npm audit` covers `web/` only). Both services now pass it, reached in the order the suppression policy demands:
> 1. **Upgrade what can be upgraded.** Six pins ahead of the Spring Boot BOM, in the "Security pins" block of both poms: **Tomcat 10.1.60, Netty 4.1.138.Final, PostgreSQL 42.7.13, httpcore5 5.4.3, httpclient5 5.6.4, log4j2 2.26.1**. Each comment names the CVE it clears. **Remove each pin when Spring Boot catches up** — otherwise it silently holds a library *below* the BOM later, which is exactly what the old `netty.version 4.1.115` pin did.
> 2. **Suppress only the genuinely unfixable.** 14 Spring Framework / Spring Security CVEs where `spring-core` 6.2.19 and `spring-security-core` 6.5.11 are the newest published versions and **no fix exists in any line**. Each entry in `docs/owasp-suppressions.xml` names its exact CVEs, carries a reachability justification, and expires `until="2026-12-22Z"` — after which the build fails again, which is the point.
>
> **Two traps when writing these.** (a) **Check both advisory sources.** GitHub's Advisory DB listed _no_ fix for `httpclient5` CVE-2026-71290, but NVD's range ends at **5.6.4**, which is published — trusting GHSA alone would have suppressed a fixable CVE. (b) **dependency-check spreads a CPE-matched CVE across every sibling jar in the group**, so suppressing `spring-core` only moved the failure to `spring-tx`. Spring entries therefore use a group-level PURL, kept narrow by listing exact CVEs.
>
...
> **⚠️ Steps 3–6 have never run against a cluster — none is configured _(Session 125 cont. 10, 2026-09-23)_.** Images build and push (first success: `ad374f9`), but `KUBE_CONFIG_PROD` / `KUBE_CONFIG_STAGING` are not set; each deploy job detects that, emits a `Deploy skipped` notice and reports green. The deploy jobs **were broken in seven ways** (skip propagation meant `deploy-production` could never run; the `sed` hit a non-existent file with the wrong pattern, a 40-char instead of 7-char SHA and a mixed-case registry path; backend `kubectl apply` wasn't recursive; the rollout waited on the wrong Deployment names) — **all fixed in cont. 10**, with the image step failing closed rather than deploying `:latest`. Still needed before a first deploy: a cluster, a one-time bootstrap (namespace, secrets, configmaps, data stores), the credentials, and SonarCloud's `SONAR_TOKEN` + `SONAR_ORG`. **Full pick-up list: `docs/deferred-backlog.md` items 8 (Kubernetes) and 9 (SonarCloud).**

1. Docker image built and pushed to GHCR as `sha-<7-char>` + branch tag
2. Trivy scans the image and uploads SARIF to the Security tab — **report-only** (`exit-code: '0'`); it does not block
3. Deploy job checks for its `KUBE_CONFIG_*` secret; if absent it skips cleanly
4. `infrastructure/k8s/<svc>/<svc>-deployment.yaml` image pinned to `ghcr.io/<lowercase repo>/cba-<svc>:sha-<7>` — **fails the job if the line wasn't rewritten**
5. `kubectl apply -f infrastructure/k8s/<svc>/` — the service's own directory **only**. Deliberately not `-R` on the whole tree: that would push `secrets/` with its `<CHANGE_ME>` placeholders over real cluster secrets. Namespace/secrets/configmaps/data stores are a one-time bootstrap.
6. `kubectl rollout status deployment/<svc>` (Deployments are named `backend` / `card-service`, not `cba-*`) waits up to 300s
7. GitHub environments (`staging`, `production`) provide deployment protection rules — **`staging` doesn't exist yet, and `production` has no required reviewers**, so once `KUBE_CONFIG_PROD` is set every push to main deploys to production
