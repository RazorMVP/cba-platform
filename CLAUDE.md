...
>
> **⚠️ OWASP Dependency Check — NVD cache must use restore/save split _(Session 125 cont. 5)_.** Plain `actions/cache` saves only when the **job succeeds**, and `owasp-check` fails on any CVSS ≥ 7 finding, so the NVD cache was **never written**. Every card-service run then paged all ~381k NVD records through the API (~40 min; sometimes never finished). Both `backend-ci.yml` and `card-service-ci.yml` now use `actions/cache/restore` + `actions/cache/save` with `if: always()`, a per-day key `owasp-nvd-Linux-YYYY-MM-DD` shared by both jobs, the project's public NVD mirror (`-DnvdDatafeedUrl=https://dependency-check.github.io/DependencyCheck_Builder/nvd_cache/`), the plugin pinned at `13.0.0`, and a 45-minute timeout. Two more traps: `hoursToWaitBeforeChecking` **is not a real property** (it's silently ignored; use `nvdValidForHours`), and the OSS Index analyzer now needs credentials, so it's disabled explicitly. **⚠️ `owasp-check` is now ADVISORY (`continue-on-error: true`), and `docker` no longer gates on it _(Session 125 cont. 7)_.** The reason is not convenience: on Spring Boot 3.5.16 the remaining findings are against the **newest published version in each line** and several have **no fix anywhere** — `spring-core` 6.2.19 (CVE-2026-47884/47890/47891/47892, 9.8s), `spring-security-core` 6.5.11 (CVE-2026-59270, 9.1), `httpclient5` (CVE-2026-71290, 9.1). Verified against the GitHub Advisory DB's exact ranges, so they are **not** CPE false positives — `-DfailBuildOnCVSS=7` is simply unsatisfiable today. Keeping it blocking meant **no backend or card-service image was built on main from July 2026 onward**. The scan still runs on every push and uploads its HTML report. **⚠️ Know the cost — this was the ONLY gate that failed a build on a CVE in an _existing_ Java dependency.** Contrary to what the security-testing table above implies, `snyk`'s scan steps are `continue-on-error: true` and the Trivy scans set no `exit-code`, so **both only report**; Dependabot raises alerts and is not a CI gate. What still blocks after this change: **`dependency-review`** (PRs only, and only dependencies *changed in that PR*) and **`npm audit`** (`web/` only, not Java). Restoring a real gate means narrow, time-boxed per-CVE suppressions in `docs/owasp-suppressions.xml` with the threshold left at 7. **Revisit — remove `continue-on-error` — once upstream ships the fixes.** Some findings _are_ patchable ahead of Spring Boot's own release (Tomcat 10.1.58+, Netty 4.1.136+, PostgreSQL 42.7.12+) and are pinned separately.
>
